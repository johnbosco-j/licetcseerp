#!/bin/bash
# ══════════════════════════════════════════════════════════════
#  Excelsior ERP — HOD Curriculum Management Module
#  Adds: subject add / edit / remove per section+semester (HOD only)
#        faculty assignment per subject
#        semester toggle (odd/even within academic year)
#  Run from project root: cd ~/Documents/Excelsior/web
#  Then: bash add-curriculum-module.sh
# ══════════════════════════════════════════════════════════════
...
echo -e "\n${CYAN}══════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}  Excelsior ERP — Curriculum Management Module${NC}"
echo -e "${CYAN}══════════════════════════════════════════════════════${NC}\n"

[ ! -f "package.json" ] && echo -e "${RED}✗ Run from ~/Documents/Excelsior/web${NC}" && exit 1

# ──────────────────────────────────────────────────────────
#  STEP 1 — Add faculty_id column to subjects via SQL
#  Print SQL for HOD to run in Supabase SQL Editor
# ──────────────────────────────────────────────────────────
echo -e "▶ Step 1 — SQL migration (run this in Supabase SQL Editor)\n"
echo -e "${YELLOW}┌─────────────────────────────────────────────────────┐${NC}"
echo -e "${YELLOW}│ Open: supabase.com/dashboard/project/               │${NC}"
echo -e "${YELLOW}│       cvazyvdxnjmoenasavvq/editor                   │${NC}"
echo -e "${YELLOW}│ Paste and run the SQL below:                        │${NC}"
echo -e "${YELLOW}└─────────────────────────────────────────────────────┘${NC}"
cat << 'SQLEOF'

-- Add faculty_id to subjects table (nullable, references profiles)
...
-- Ensure HOD can manage subjects (insert/update/delete)
DROP POLICY IF EXISTS "subjects_write" ON subjects;
CREATE POLICY "subjects_write" ON subjects
  FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Verify
SELECT id, code, name, semester, section, credits, faculty_id
FROM subjects LIMIT 3;

SQLEOF
echo ""

# ──────────────────────────────────────────────────────────
#  STEP 2 — Add "curriculum" to roles.ts HOD nav
# ──────────────────────────────────────────────────────────
echo -e "▶ Step 2 — Adding 'curriculum' to HOD nav in roles.ts"

if grep -q '"curriculum"' src/lib/roles.ts; then
  echo -e "  ${YELLOW}⚠ Already present${NC}"
else
  sed -i '' 's/"reports"/"curriculum","reports"/' src/lib/roles.ts
  echo -e "  ${GREEN}✓ curriculum added to NAV_HOD_IDS${NC}"
fi

# ──────────────────────────────────────────────────────────
#  STEP 3 — Create the curriculum page route
# ──────────────────────────────────────────────────────────
echo -e "▶ Step 3 — Creating src/app/dashboard/curriculum/page.tsx"
mkdir -p src/app/dashboard/curriculum

cat > src/app/dashboard/curriculum/page.tsx << 'PAGEEOF'
"use client"
...
PAGEEOF
echo -e "  ${GREEN}✓ page route created${NC}"

# ──────────────────────────────────────────────────────────
#  STEP 4 — Create the full curriculum module component
# ──────────────────────────────────────────────────────────
echo -e "▶ Step 4 — Creating curriculum-module.tsx"
mkdir -p src/components/modules

cat > src/components/modules/curriculum-module.tsx << 'MODEOF'
"use client"
...
export default function CurriculumModule() {
  ...
  const [semParity, setSemParity]       = useState<0 | 1>(0) // 0=odd(Jul-Nov), 1=even(Jan-May)
...
echo -e "  ${GREEN}✓ src/components/modules/curriculum-module.tsx created${NC}"

# ──────────────────────────────────────────────────────────
#  STEP 5 — Add Curriculum nav item to layout.tsx
# ──────────────────────────────────────────────────────────
echo -e "▶ Step 5 — Adding Curriculum to sidebar nav"

if grep -q '"curriculum"' src/app/dashboard/layout.tsx 2>/dev/null; then
  echo -e "  ${YELLOW}⚠ Already present in layout${NC}"
else
  # Add GraduationCap to lucide import if not present
  sed -i '' 's/import {/import { GraduationCap,/' src/app/dashboard/layout.tsx 2>/dev/null || true

  # Add nav item after Reports entry
  python3 - << 'PYEOF'
path = "src/app/dashboard/layout.tsx"
with open(path) as f:
    code = f.read()

# Add nav item after the Reports entry
old = '{ icon: FileBarChart,    label: "Reports",          id: "reports" },'
new = '{ icon: FileBarChart,    label: "Reports",          id: "reports" },\n  { icon: GraduationCap,   label: "Curriculum",       id: "curriculum" },'
code = code.replace(old, new)

with open(path, 'w') as f:
    f.write(code)
print("  layout.tsx nav updated")
PYEOF
  echo -e "  ${GREEN}✓ Curriculum added to sidebar${NC}"
fi

# ──────────────────────────────────────────────────────────
#  STEP 6 — Clear cache and restart
# ──────────────────────────────────────────────────────────
echo -e "▶ Step 6 — Clearing cache and restarting"
pkill -f "next dev" 2>/dev/null || true
rm -rf .next

echo -e "\n${GREEN}══════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  Done! Curriculum module is ready.${NC}"
echo -e "${GREEN}══════════════════════════════════════════════════════${NC}"
echo -e ""
echo -e "  ${YELLOW}⚠ IMPORTANT — Run this SQL in Supabase first:${NC}"
echo -e "  ${YELLOW}  supabase.com/dashboard/project/cvazyvdxnjmoenasavvq/editor${NC}"
echo -e ""
echo -e "  ALTER TABLE subjects"
echo -e "    ADD COLUMN IF NOT EXISTS faculty_id UUID REFERENCES profiles(id) ON DELETE SET NULL;"
echo -e ""
echo -e "  ${GREEN}Then:${NC}"
echo -e "  1. npm run dev"
echo -e "  2. Log in as HOD"
echo -e "  3. Click 'Curriculum' in the sidebar"
echo -e "  4. Pick a section + semester → Add / Edit / Remove subjects"
echo -e "  5. Click a row to expand and change faculty assignment\n"
