    ...
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-semibold tracking-tight mt-2">Document Repository</h1>
        ...
        {CATEGORIES.map(cat => {
          ...
          return (
            <button key={cat} onClick={() => setFilter(cat)}
              className={`text-[12.5px] font-medium px-3.5 py-1.5 rounded-full border transition-all ${filter === cat ? 'bg-primary text-primary-foreground border-primary' : 'border-border text-muted-foreground hover:border-primary/50'}`}>
      ...
      {Object.keys(grouped).length === 0 ? (
        ...
        <div key={cat} className="bg-card border border-border rounded-lg">
          <div className="px-6 py-4 border-b border-border bg-licet-paper/70 rounded-t-xl">
            ...
            <h2 className="font-serif text-[19px] font-semibold text-licet-indigo mt-1">{CAT_ICONS[cat]} {cat} ({docs.length})</h2>
          </div>
            ...
            {docs.map(doc => (
              <div key={doc.id} className="flex items-center gap-4 px-6 py-4 hover:bg-accent/30 transition-colors">
                ...
                {doc.isStorageFile ? (
                  ...
                  <a href={safeUrl(doc.url)} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1 px-3 py-1.5 bg-primary/10 text-primary font-mono text-xs rounded border border-primary/20 hover:bg-primary/20 transition-colors flex-shrink-0">
