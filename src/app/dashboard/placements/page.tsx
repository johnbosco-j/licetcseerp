            ...
            <div key={p.id} className="bg-card border border-border rounded-lg p-4 opacity-60 hover:opacity-80 transition-all">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="font-mono text-xs text-muted-foreground">{p.role_title} {p.package_lpa ? `· ₹${p.package_lpa} LPA` : ''}</p>
                </div>
                {isHOD && (
                  <button onClick={() => toggleActive(p.id, p.is_active)}
                    className="font-mono text-xs px-2 py-1 border border-border rounded hover:border-primary/50 text-muted-foreground">
                    Reactivate
                  </button>
                )}
              </div>
