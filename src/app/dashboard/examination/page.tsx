  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-1 border-b border-border">
        {([
          ...
          <button key={key} onClick={() => setTab(key)}
            className={`flex items-center gap-2 px-4 py-2.5 font-mono text-xs border-b-2 transition-all ${tab === key ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
            ...
            <>
              ...
              <div className="bg-card border border-border rounded-lg">
                ...
                {papers.length === 0 ? (
                  ...
                  <div key={paper.id} className="flex items-center gap-4 px-6 py-4 border-b border-border last:border-0 hover:bg-accent/30 transition-colors">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        ...
                        <span className="font-mono text-xs text-muted-foreground">{paper.exam_type}</span>
                        <span className="font-mono text-xs text-muted-foreground">{paper.section}</span>
                      </div>
                      <p className="font-medium text-sm">{paper.subject_code} — {paper.subject_name || paper.title}</p>
                      <p className="font-mono text-xs text-muted-foreground">{new Date(paper.created_at).toLocaleDateString()}</p>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <button onClick={() => { setEditing(paper); setContent(paper.content ?? QP_TEMPLATE) }}
                        className="font-mono text-xs px-3 py-1.5 border border-border rounded hover:border-primary/50 transition-colors">
                        Open
                      </button>
                      {isHOD && paper.status === 'SUBMITTED' && (
                        <>
                          ...
                          <button onClick={() => updateQPStatus(paper.id, 'REJECTED', JSON.stringify(paper))}
                            className="flex items-center gap-1 font-mono text-xs px-3 py-1.5 bg-red-600 text-white rounded hover:bg-red-700">
      ...
      {tab === 'exam_schedule' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="font-mono text-xs text-muted-foreground">{schedule.length} scheduled exams</p>
...
function MarksAnalysisCard({ subject, section }: { subject: Subject; section: string }) {
  ...
  if (!stats) return (
    <div className="bg-card border border-border rounded-lg p-4">
      <p className="font-mono text-xs text-muted-foreground">{subject.code} — {subject.name}</p>
      <p className="font-mono text-xs text-muted-foreground mt-2">No marks data</p>
    </div>
  )
        ...
        {Object.entries(stats.grades).map(([g, count]) => (
          Number(count) > 0 && (
            <div key={g} className="flex-1 text-center">
              ...
              <p className="font-mono text-xs text-muted-foreground">{g}</p>
            </div>
          )
        ))}
      </div>
